...
def run_LRP_HAI(sess, net, blob, timers, mode, beta, im_idx=None,
    ...
    if im_idx is not None and obs_canvas_all is not None:
        cls_det = save_visualization(im_blob, im_shape, im_idx, obs_canvas_all, scores,
    ...
    if mode == 'test':
        if extra_args is not None:
            return cls_det, scores, pred_bboxes, timers, [t, frac_area]
        else:
            return cls_det, scores, pred_bboxes, timers, [t, frac_area]
    else:
...
def save_visualization(im_blob, im_shape, im_idx, obs_canvas, cls_probs,
    ...
    cls_det = {}
    for i in range(len(names_and_coords)):
        if names_and_coords[i]['class_name'] == 'uvula':
            cls_det = cls_dets[i]
