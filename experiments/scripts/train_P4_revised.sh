...
if [[ ! -z  ${EXTRA_ARGS_SLUG}  ]]; then
  NET_FINAL=fr-rcnn-weights/P4/${NET}/${TRAIN_IMDB}/${EXTRA_ARGS_SLUG}/res101_faster_rcnn_iter_${ITERS}.ckpt
else
  NET_FINAL=fr-rcnn-weights/P4/${NET}/${TRAIN_IMDB}/default/res101_faster_rcnn_iter_${ITERS}.ckpt
fi
set -x
...
if [ ! -f ${NET_FINAL}.index ]; then
  if [[ ! -z  ${EXTRA_ARGS_SLUG}  ]]; then
    CUDA_VISIBLE_DEVICES=${GPU_ID} time python ./tools/trainval_net_fr.py \
      --weight for_transfer_frcnn_weights/P4/${NET}/${TRAIN_IMDB}/default/res101_faster_rcnn_iter_${ITERS}.ckpt \
      --imdb ${TRAIN_IMDB} \
      --imdbval ${TEST_IMDB} \
      --iters ${ITERS} \
